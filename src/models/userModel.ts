...
export const updateUserById = async (
  ...
  try {
    //사용자 데이터 업데이트
    const [result] = await connection.query<OkPacket>('UPDATE user SET ? WHERE user_id = ?', [updateData, userId]);

    if (!result.affectedRows) {
      return null;
    }
    //업데이트된 사용자 데이터 가져옴
    const [updatedUserRows] = await connection.execute('SELECT * FROM user WHERE user_id = ?', [userId]);
...
export const deleteUserById = async (userId: string): Promise<boolean> => {
  ...
  try {
    const [result] = await connection.query<OkPacket>('DELETE FROM user WHERE user_id = ?', [userId]);
