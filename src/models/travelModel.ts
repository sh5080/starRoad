...
export const getTravelLocationsByPlanId = async (planId: number): Promise<TravelLocation[]> => {
  try {
    ...
    if (error instanceof AppError) {
      ...
      console.error(error);
      throw new AppError(CommonError.UNEXPECTED_ERROR, '날짜별 장소조회에 실패했습니다.', 500);
    }
  }
};
...
export const getTravelPlanDetailsByPlanId = async (planId: string): Promise<TravelPlan> => {
  try {
    ...
    if (error instanceof AppError) {
      ...
      console.error(error);
      throw new AppError(CommonError.UNEXPECTED_ERROR, '일정 상세조회에 실패했습니다.', 500);
    }
  }
};
...
export const updateTravelLocation = async (
  ...
  try {
    ...
    if (rows.length === 0) {
      throw new AppError(CommonError.RESOURCE_NOT_FOUND, 'locationId를 찾을 수 없습니다.', 404);
    }
    ...
    if (error instanceof AppError) {
      ...
      console.error(error);
      throw new AppError(CommonError.UNEXPECTED_ERROR, '일정 수정에 실패했습니다.', 500);
    }
  } finally {
...
export const deleteTravelPlan = async (
  ...
  try {
    ...
    if (error instanceof AppError) {
      ...
      console.error(error);
      throw new AppError(CommonError.UNEXPECTED_ERROR, '일정 삭제에 실패했습니다.', 500);
    }
  } finally {
    connection.release();
  }
